...
GameState convertNodeToBoard(struct TreeNode* node);
...
char compareStates(GameState g1, GameState g2);
...
void destroyExcessBranches(struct TreeNode* root, char move);
void destroyBranch(struct TreeNode* destroy, struct TreeNode* parent);
void resetValues(struct TreeNode* node);
void resetTree(struct TreeNode* root);
...
void flipChildren(struct TreeNode* node, char action);
...
void printState(GameState gstate);
char getAction(GameState gstate);
char* availableActions(GameState gstate);
char canDoAction(GameState gstate, char action);
char terminalState(GameState gstate);
GameState doAction(GameState gstate, char action);
char checkHorizontal(GameState gstate, char i, char j, char player);
char checkVertical(GameState gstate, char i, char j, char player);
char checkDiagonal_DL_UR(GameState gstate, char i, char j, char player);
char checkDiagonal_UL_DR(GameState gstate, char i, char j, char player);
char isColumnLeveled(GameState gstate, char col);

/*Only used for debug*/
int count_nodes(struct TreeNode* root) {
	if(root->counted) return 0;
	root->counted = 1;
	int sum = 0;
	for(int i = 0; i < 21; i++) {
...
char deep(struct TreeNode* node) {
	node->counted = 0;
	char max = node->turn_from_root;
	for(char i = 0; i < 21; i++) {
		if(node->children[i] == NULL) continue;
		char child_deep = deep(node->children[i]);
		if(max < child_deep) max = child_deep;
	}
	return max;
}
...
void destroyLinkedList(struct LinkedNode* node) {
	if(node->left) destroyLinkedList(node->left);
	if(node->right) destroyLinkedList(node->right);
	free(node);
}
...
int main(int argc, char** argv) {
	...
	if(iflag && oflag) {
		fprintf(stderr, "Usage: [./connect4x4 -i] OR [./connect4x4 -o] OR  [./connect4x4]");
		exit(-1);
	}
	time_t seed = time(NULL);//1543056436;//time(NULL);
	printf("Seed: %lld\n", (long long) seed);
	srand(seed);
	struct TreeNode* root;
	if(iflag) {
	...
	while(!terminalState(gstate)) {
		...
		if((action >= 7 && action < 14) && isColumnLeveled(gstate, action - 7)) { //If enemy moved where AI decided not to look (right side on level ground)
		...
		printf("Tree Before: %d - %d\n", count_nodes(root), deep(root));
		destroyExcessBranches(root, action);
		printf("Tree After:  %d - %d\n", count_nodes(root), deep(root));
...
void flipChildren(struct TreeNode* node, char action) { //Assumes action >= 7 && action < 14
        ...
        for(int i = 0; i < 14; i++) { //Don't take blockers into account, they level ground, thus not needed for flipping
                if(node->children[i] == NULL) continue;
...
void getDuplicates(struct TreeNode** node_created, struct TreeNode* node_check, char* moves_to_check, char* moves_upto_node, GameState g_created, GameState g_check) {
	...
	if(compareStates(g_created, g_check)) { //If two states are actually the same
		...
		(*node_created)->parents = (struct TreeNode**) 0x22222222;
		free((*node_created)->move_after_parent);
		(*node_created)->move_after_parent = (char*) 0x22222222;
		free((*node_created));
